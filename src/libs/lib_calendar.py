...
import pieces
"""
Todos processos referentes a busca das datas do calendario e calculo de dias uteis
"""
...
def get_dias_uteis(mes, ano, feriados=[]):
    try:
        ...
        pieces.lib_logging.logger.info(f" > Total Dias Uteis: {len(dias_uteis_sem_feriados)}, dias_uteis: {dias_uteis_sem_feriados}") 
        continuar = True  
...
def valida_data_dataframe(data):
    """Extrai o dia como inteiro de diferentes tipos:
       - pd.Timestamp
       - str (dd/mm/yyyy)
       - None, NaN, NaT => retorna None
    """
    if data is None or (isinstance(data, float) and pieces.math.isnan(data)) or pieces.pd.isna(data):
        return None
    elif isinstance(data, pieces.pd.Timestamp):
        return data.day
    elif isinstance(data, str):
        try:
            return pieces.datetime.strptime(data, "%d/%m/%Y").day
        except ValueError:
            raise ValueError(f"String de data em formato incorreto: {data}")
    else:
        raise TypeError(f"Tipo não suportado para dia: {type(data)}")